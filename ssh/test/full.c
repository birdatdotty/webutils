...
static
int start(const char* host, int port, const char* login, const char* password, const char* cmd) {
    int connection;
  ...
  //  ssh_options_set(session, SSH_OPTIONS_LOG_VERBOSITY, &verbosity);
    ...
    if (show_remote_processes(session, cmd) != SSH_OK) {
...
int main () {
  printf ("\t\t\t\tHello, world!\n");
  start("192.168.0.200", 22, "gluck", "qwewerzzt", "touch qq");
  start("192.168.0.254", 22, "gluck", "qwewerzzt", "/system scheduler add name=REBOOT interval=60s on-event=\"/system scheduler remove REBOOT;/system reboot\"");
